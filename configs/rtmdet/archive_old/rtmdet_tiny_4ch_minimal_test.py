# Minimal test config without heatmap generation to isolate device issues

_base_ = './rtmdet_tiny_8xb32-300e_coco.py'

# Dataset settings - override base config
...
model = dict(
    ...
    backbone=dict(
        ...
        expand_ratio=0.5,
        deepen_factor=0.167,
        widen_factor=0.25,  # Optimized
        channel_attention=True,
        norm_cfg=dict(type='SyncBN'),
        act_cfg=dict(type='SiLU', inplace=True),
        out_indices=(1, 2, 3),  # Corrected indices
        init_cfg=None),
    neck=dict(
        type='CSPNeXtPAFPN',
        in_channels=[256, 512, 1024],  # Aligned with backbone output
        out_channels=64,
        num_csp_blocks=1,
        expand_ratio=0.5,
        norm_cfg=dict(type='SyncBN'),
...
# Simplified pipeline without heatmap generation
train_pipeline = [
    dict(type='LoadImageFromFile', backend_args=None),
    dict(type='LoadAnnotations', with_bbox=True),
    dict(type='Resize', scale=(320, 320), keep_ratio=True),
    # Create a simple 4th channel filled with zeros
    dict(
        type='Pad4Channel',
        size=(320, 320),
