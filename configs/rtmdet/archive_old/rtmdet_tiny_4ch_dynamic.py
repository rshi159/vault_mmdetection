...
train_pipeline = [
    ...
    dict(
        ...
        # YOLO keypoint data access
        yolo_dataset_path='/home/robun2/Documents/vault_conveyor_tracking/vault_mmdetection/development/augmented_data_production'
    ),
    
    # Simplified 4-channel compatible pipeline (bypassing RTMDet augmentations)
    dict(
        type='RandomResize',
        scale=(1280, 1280),
    ...
    # Final packaging - include all required metadata keys
    dict(type='PackDetInputs', meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape', 'scale_factor', 'pad_shape', 'pad_fixed_size', 'pad_size_divisor'))
]
...
val_pipeline = [
    ...
    dict(
        ...
        keypoint_noise_std=8.0,    # Less keypoint noise
        quality_variance=False,    # More consistent in validation
        min_sigma=12.0,            # Narrower quality range
        max_sigma=30.0,
    ...
    dict(
        ...
        meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape',
                   'scale_factor', 'pad_shape', 'pad_fixed_size', 'pad_size_divisor'))
]
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'valid/annotations.json',  # Correct path to validation annotations
    metric='bbox',
    format_only=False
)
...
# Conservative learning rate schedule (epoch-based for simplicity and reliability)
param_scheduler = [
    dict(
        type='LinearLR', 
        start_factor=0.001, 
        by_epoch=True, 
        begin=0, 
        end=3),  # 3 epoch warmup
    dict(
        type='CosineAnnealingLR',
        eta_min=0.00005,           # Lower minimum LR
        begin=3,
        end=450,                   # Total epochs
        T_max=447,                 # 450 - 3 epochs
        by_epoch=True)
]
