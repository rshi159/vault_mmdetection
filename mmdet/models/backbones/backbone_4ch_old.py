...
class CSPNeXt4Ch(nn.Module):
    ...
    def __init__(self, 
        ...
        # Stem: 4-channel input -> match original structure
        # Original checkpoint has 12 output channels from first conv with widen_factor=0.375
        # This suggests stem output should be 32 * widen_factor = 12 (for tiny)
        stem_channels = max(8, int(32 * widen_factor))  # 12 for tiny (0.375), matches checkpoint
        self.stem = BasicBlock(4, stem_channels, 3)  # 4 -> 12 (matches checkpoint structure)
        self.stem2 = BasicBlock(stem_channels, channels[0], 3)  # 12 -> 96
