...
"""4-channel CSPNeXt backbone implementation for RTMDet.

This module provides a native 4-channel backbone that processes RGB + Heatmap
input without information loss. The implementation extends the standard CSPNeXt
architecture to handle the additional channel while maintaining compatibility
with existing RTMDet configurations.

Key features:
    - Native 4-channel input processing
    - Compatible with RTMDet tiny/small/medium variants
    - Proper weight initialization for additional channel
    - Memory efficient implementation
    - No dependency on pretrained 3-channel weights

Example:
    Basic usage:
        backbone = CSPNeXt4Ch(
            arch='P5',
            widen_factor=0.375,    # RTMDet-tiny
            deepen_factor=0.167,   # RTMDet-tiny
            out_indices=(2, 3, 4)  # P3, P4, P5 outputs
        )
        
        # Process 4-channel input (RGB + Heatmap)
        features = backbone(input_4ch)  # Shape: (B, 4, H, W)
